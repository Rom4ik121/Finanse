...
def _settings_section(
    ...
    """Card block with icon title and stacked controls."""
    return card_surface(
        ft.Column(
            spacing=12,
            tight=True,
...
class SettingsPage(ft.Column):
    ...
    def __init__(self, page: ft.Page, state: "AppState") -> None:
        ...
        self._currency = ft.Dropdown(
            label=tr("settings.default_currency", lang),
            value=normalize_currency_code(s.default_currency),
            options=currency_dropdown_options(lang=lang, include_crypto=False),
            expand=True,
            dense=True,
        )
        ...
        self._debt_reminders = ft.Switch(
            label=tr("debts.reminders", lang),
            value=s.debt_reminders,
        )
        self._sub_reminders = ft.Switch(
            label=tr("notify.subscription_due", lang),
    ...
    async def wipe_all_data(self) -> None:
        ...
        try:
            ...
            if c.get_settings is not None:
                ...
                self._state.set_settings(settings, notify=False)
                apply_theme(self._page, settings.theme)
            from lib.main import _seed_categories

            await _seed_categories(c)
            if c.create_account is not None:
                ...
                await c.create_account.execute(
                    Account(
                        ...
                        currency=c.config.default_currency,
                        initial_balance=0,
                        balance=0,
                        icon="wallet",
                        color="#2E7D32",
